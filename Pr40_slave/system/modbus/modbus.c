...
void modbus_read_reg_func(unsigned int addr, unsigned int num)
{
  ...
  if(addr>=REG_SENSOR1_RECORD_VALUE) return; //на лету вычисляем только первые 0x64 регистра
...
void modbus_write_reg_func(unsigned int addr,unsigned int value)
{
  switch (addr)
  {
    ...
    case REG_CONFIG_K9:  
    case REG_CONFIG_K10:  
      slave_regs[addr]=value; //dml!!! проверку на допустимый диапазон добавить тут?
      break;
